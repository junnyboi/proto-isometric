...
const PhaseNineTestsScript: GDScript = preload(
	"res://test/test_harvest_settlement_phase_nine.gd"
)
const SAVE_PATH: String = "/tmp/protos-harvest-settlement-phase-nine.json"
...
func _run() -> void:
	...
	for _frame: int in 16:
		await process_frame
	for test_case: Dictionary in PhaseNineTestsScript.evaluate(runtime):
		_check(test_case)
	for test_case: Dictionary in await PhaseNineTestsScript.evaluate_responsive(runtime):
		_check(test_case)
	runtime.free()
	await process_frame
	...
	for _frame: int in 16:
		await process_frame
	for test_case: Dictionary in PhaseNineTestsScript.evaluate_reloaded(reopened):
		_check(test_case)
	reopened.free()
	await process_frame
	_remove_save_files()
	if _failures == 0:
		print("[HARVEST_SETTLEMENT_PHASE_NINE_PASS] checks=%d" % _checks)
		quit(0)
	else:
		print(
			"[HARVEST_SETTLEMENT_PHASE_NINE_FAIL] checks=%d failures=%d"
			% [_checks, _failures]
		)
		quit(1)
