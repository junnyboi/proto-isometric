...
const HarvestPhaseFourTestsScript: GDScript = preload("res://test/test_harvest_phase_four.gd")
const SAVE_PATH: String = "/tmp/protos-harvest-phase-four.json"
...
func _run() -> void:
	...
	await process_frame
	await process_frame
	for test_case: Dictionary in HarvestPhaseFourTestsScript.evaluate(runtime):
		_check(test_case)
	runtime.free()
	await process_frame
	_remove_save_files()
	if _failures == 0:
		print("[PHASE_FOUR_PASS] checks=%d" % _checks)
		quit(0)
	else:
		print("[PHASE_FOUR_FAIL] checks=%d failures=%d" % [_checks, _failures])
		quit(1)
