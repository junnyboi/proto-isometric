...
func _test_isometric_map() -> void:
	...
	_check(world_objects.has_method("_draw_destructible"), "generated rocks use object renderer")
	_check(
		(
	...
	_check(field_hud.has_node("ContextTutorialPresenter"), "context tutorial presenter exists")
	...
	_check(not bool(map.call("has_destructible_rock", Vector2i(4, 4))), "rock disappears immediately")
	_check(bool(map.call("is_walkable", Vector2i(4, 4))), "blocking clears immediately")
	_check(bool(heat_haze.call("has_haze_at", Vector2i(4, 4))), "salvaged sand enables haze")
	_check(not bool(map.call("has_scrap", Vector2i(4, 4))), "magnet collects rock scrap immediately")
	_check(int(effects.call("get_emission_count")) == 1, "accepted Smash emits debris immediately")
	_check(int(effects.call("get_particle_count")) >= 20, "rock impact emits debris particles")
	_check(float(effects.call("get_shake_remaining")) > 0.0, "accepted Smash starts camera shake")
	...
	avatar.call("_process", float(avatar.call("get_attack_contact_time")) + 0.01)
	_check(int(effects.call("get_emission_count")) == 1, "contact frame emits no duplicate debris")
	_check(
