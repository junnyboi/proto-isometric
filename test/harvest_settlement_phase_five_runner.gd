...
const PhaseFiveTestsScript: GDScript = preload(
	"res://test/test_harvest_settlement_phase_five.gd"
)
const SAVE_PATH: String = "/tmp/protos-harvest-settlement-phase-five.json"
...
func _run() -> void:
	...
	for _frame: int in 14:
		await process_frame
	var first_cases: Array[Dictionary] = PhaseFiveTestsScript.evaluate(runtime)
	...
	for _frame: int in 14:
		await process_frame
	var reload_cases: Array[Dictionary] = PhaseFiveTestsScript.evaluate_reloaded(reopened)
	...
	_remove_save_files()
	if _failures == 0:
		print("[HARVEST_SETTLEMENT_PHASE_FIVE_PASS] checks=%d" % _checks)
		quit(0)
	else:
		print(
			"[HARVEST_SETTLEMENT_PHASE_FIVE_FAIL] checks=%d failures=%d"
			% [_checks, _failures]
		)
		quit(1)
