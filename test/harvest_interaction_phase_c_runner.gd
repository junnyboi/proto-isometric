...
const PhaseCTestsScript: GDScript = preload("res://test/test_harvest_interaction_phase_c.gd")
const SAVE_PATH: String = "/tmp/protos-harvest-interaction-phase-c.json"
...
func _run() -> void:
	...
	for _frame: int in 7:
		await process_frame
	for test_case: Dictionary in PhaseCTestsScript.evaluate(runtime):
		_check(test_case)
	runtime.free()
	await process_frame
	_remove_save_files()
	if _failures == 0:
		print("[HARVEST_INTERACTION_PHASE_C_PASS] checks=%d" % _checks)
		quit(0)
	else:
		print(
			"[HARVEST_INTERACTION_PHASE_C_FAIL] checks=%d failures=%d"
			% [_checks, _failures]
		)
		quit(1)
