...
const PhaseTenTestsScript: GDScript = preload(
	"res://test/test_harvest_settlement_phase_ten.gd"
)
const SAVE_PATH: String = "/tmp/protos-harvest-settlement-phase-ten.json"
...
func _run() -> void:
	...
	for _frame: int in 16:
		await process_frame
	for test_case: Dictionary in PhaseTenTestsScript.evaluate(runtime):
		_check(test_case)
	runtime.free()
	await process_frame
	...
	for _frame: int in 16:
		await process_frame
	for test_case: Dictionary in PhaseTenTestsScript.evaluate_reloaded(reopened):
		_check(test_case)
	reopened.free()
	await process_frame
	_remove_save_files()
	if _failures == 0:
		print("[HARVEST_SETTLEMENT_PHASE_TEN_PASS] checks=%d" % _checks)
		quit(0)
	else:
		print(
			"[HARVEST_SETTLEMENT_PHASE_TEN_FAIL] checks=%d failures=%d"
			% [_checks, _failures]
		)
		quit(1)
