...
const PhaseFivePresentationTestsScript: GDScript = preload(
	"res://test/test_harvest_phase_five_presentation.gd"
)
const SAVE_PATH: String = "/tmp/protos-harvest-phase-five-presentation.json"
...
func _run() -> void:
	...
	for _frame: int in 5:
		await process_frame
	for test_case: Dictionary in PhaseFivePresentationTestsScript.evaluate(runtime):
		_check(test_case)
	runtime.free()
	await process_frame
	_remove_save_files()
	if _failures == 0:
		print("[PHASE_FIVE_PRESENTATION_PASS] checks=%d" % _checks)
		quit(0)
	else:
		print("[PHASE_FIVE_PRESENTATION_FAIL] checks=%d failures=%d" % [_checks, _failures])
		quit(1)
