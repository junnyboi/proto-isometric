...
static func build(source: Dictionary, arguments: Dictionary) -> Dictionary:
	...
	var family: StringName = &"tree" if world_kind == &"object.tree" else &"resource"
	var canonical_reward: Dictionary = reward_for(family, source_kind)
	if not _arguments_match_authority(arguments, family, canonical_reward):
		return _result(false, source, &"invalid_world_operation")
...
static func _arguments_valid(arguments: Dictionary) -> bool:
	return (
		...
		and (
			...
			in [&"object.tree", &"object.rock", &"object.resource"]
		)
		and (
			arguments.get(&"required_tool", &"")
			in [ToolServiceScript.TOOL_AXE, ToolServiceScript.TOOL_PICK]
		)
...
static func _arguments_match_authority(
	...
	var expected_tool: StringName = (
		ToolServiceScript.TOOL_AXE if family == &"tree" else ToolServiceScript.TOOL_PICK
	)
	return (
