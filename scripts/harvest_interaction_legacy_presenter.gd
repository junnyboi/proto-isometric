...
func bind(
	...
	_dossier_coordinator: RefCounted = null,
	_sealed_anchor: Callable = Callable(),
) -> bool:
	if controller == null:
		return false
...
static func validate_layout(layout: Dictionary) -> bool:
	...
	if not valid or not layout.has(&"mode"):
		return valid
	var mode: StringName = layout[&"mode"] as StringName
	var panels: Array = layout.get(&"panel_rects", []) as Array
	for value: Variant in panels:
		if not value is Rect2 or not safe.encloses(value as Rect2):
			return false
	if mode in [LAYOUT_WIDE_TERRAIN, LAYOUT_WIDE_OBJECT]:
		var aperture: Rect2 = layout.get(&"world_aperture", Rect2()) as Rect2
		return aperture.size.x >= MIN_WORLD_APERTURE and safe.encloses(aperture)
	return mode in [LAYOUT_COMPACT_SIDE, LAYOUT_PORTRAIT_SHEET, LAYOUT_PORTRAIT_COMPACT]
