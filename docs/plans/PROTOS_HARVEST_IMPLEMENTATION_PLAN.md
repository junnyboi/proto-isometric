...
| **Phase 3 — Day-one farm proof** | **Complete** | Revision `aceb80c`; Phase 3 focused suite passed **24/24**; Phase 0 passed **83/83**; Phase 2 passed **19/19**; full smoke passed **1,981/1,981**; live save/reload, landscape/portrait Xvfb, `./verify.sh --release`, Web export, and exact PCK boot all passed | Six crops, inventory/storage, stamina, forecast, atomic day advancement, shipping, seed purchase, and one workshop upgrade form a certified first-day loop. |
| **Phase 4 — Authoritative persistence and economy** | **Complete** | Phase 4 focused suite passed **21/21**; Phase 0 passed **83/83**; Phase 2 passed **19/19**; Phase 3 passed **24/24**; full smoke passed **1,984/1,984**; 3,360-day deterministic soak, maximum-ledger save budget, landscape/portrait Xvfb, `./verify.sh --release`, Web export, and exact PCK boot all passed | Stable world mutations, compensating cross-domain transactions, exact-once machines, six durable upgrade families, hard caps, canonical ordering, and GPT Image 2 machine presentation are certified. |
| **Phase 5 — Lovable homestead** | **Complete** | Phase 5 authority suite passed **19/19**; presentation/audio/brand suite passed **13/13**; Phases 0–4 passed **83/83**, **32/32**, **19/19**, **24/24**, and **21/21**; full smoke passed **1,984/1,984**; landscape/portrait/restored-community Xvfb checks, `./verify.sh --release`, Web export, and exact PCK boot all passed | Three restored facilities, three residents, relationships and requests, three livestock species, deterministic schedules, clearing music, Protos Harvest 2.0 identity, and batched community presentation are certified. |
| **Phase 6 — Wilderness bridge** | **Complete** | Revision `22a57fd`; Phase 6 focused suite passed **15/15**; Phases 4–5 passed **21/21**, **19/19**, and **13/13**; full smoke passed **1,984/1,984**; deterministic stream/reload, exact-once return, landscape/portrait Xvfb, direct import, Web export, and exact PCK boot all passed | Twelve bounded habitats, protected friendly herds, useful loot, four forecastable hazard opportunities, the Ironjaw desert arc, and atomic expedition return are certified. |
| **Phase 7 — Certification and expansion gate** | **Complete** | `./verify.sh --release`, Web export, exact PCK boot, `pnpm check`, `pnpm build`, browser title/field readiness, runtime-console scan, and checkpoint `3c3baa9e` all passed | Existing WebDev project `proto-isometric-web` now hosts the certified Protos Harvest pack in a zero-margin, borderless, dynamic fullscreen iframe. No direct publish tool was available; the verified checkpoint is ready for the WebDev Publish control. |
...
**PH-20** adds a chunk-indexed mutation ledger with stable kind-and-anchor identifiers, multi-cell footprints, canonical chunk/stable-ID ordering, strict coordinate/value/record bounds, and duplicate/overlap rejection. Legacy destroyed/placed rock arrays and dropped/collected scrap remain exact compatibility views. A maximum 4,096-record ledger remains below the two-megabyte save budget, while record 4,097 is rejected before normalization.

**PH-21** routes live productive farm commits through one detached world/run/profile/farm envelope. Persistence failure publishes nothing; publish failure triggers compensating live and persisted rollback to the exact source; successful operations publish one validated envelope. Placement retains the persistent ledger across operations, expedition returns are exact-once, and the live bridge proves farm transactions and the cross-domain snapshot cannot diverge.

**PH-22** adds typed workbench/furnace station tags, two data-driven recipes, deterministic start/complete/claim machine state, reload-safe operation tokens, and bounded claim-token history. Inputs are consumed once, outputs are credited once, same-day replay is rejected, incompatible stations cannot consume inventory, and orphaned machine records fail schema validation.

**PH-23** defines tool, robot, storage, irrigation, safehouse, and machine upgrade families with strict costs, prerequisites, and stable capability IDs. The legacy watering-efficiency price/material contract remains exact; robot capacity, storage slots, safehouse power, irrigation radius, and furnace installation mutate only inside detached candidates. GPT Image 2 furnace and irrigation props render only when their durable state is present.

**PH-24** tightens production caps across plots, inventories, machines, facilities, residents, ruins, upgrades, ecology, coordinates, ledgers, and token rings. Equivalent inputs serialize canonically, invalid duplicate/orphaned/oversized records are rejected, and two independent 3,360-day simulations produce byte-identical valid farm state below the repository's two-megabyte limit.

### Phase 5 implementation evidence

**PH-25** turns the repaired home and three nearby ruins into persistent facility records with fixed footprints, costs, power state, exact-once repair/power tokens, service gating, and bidirectional ruin-registry reconciliation. The live adjacent resolver now treats facility cells as structures and routes repair before power through the existing cross-domain transaction boundary.

**PH-26 / PH-27** add Lyra, Rook, and Mira as stable resident definitions with clear-weather and rain schedules, facility-gated arrivals, one-talk-per-day tokens, deterministic gifts, relationship hearts, and one active bounded request per resident. All service mutations validate detached farm candidates; unknown IDs, duplicate records, invalid schedule cells, oversized token rings, and persistence failures are rejected without partial publication.

**PH-28** adds Mossback Grazer, Coilhen, and Rustsnout Rooter authorities with strict housing, capacity, feed, product, bond, and daily care-token rules. GPT Image 2 anchor frames drove locked-camera hot-pink video carriers; deterministic extraction produced 4×2 eight-frame atlases. Live presentation uses atlas regions and stable diagonal depth without allocating per-animal nodes.

**PH-27 / PH-28** compose the generated repaired home, greenhouse, workshop, clinic/kitchen, resident portraits, and livestock sheets through immutable chunk-indexed records consumed by the existing batch renderer. Existing responsive HUD, target snap, touch-safe controls, onboarding overlay, zoom, remapping, bilingual UI, and accessibility preferences remain active around the new homestead records. Legacy ruin art remains visible until restoration and is suppressed only where generated homestead records own the same cells. Landscape, portrait, action, and fully restored community captures show no fallback textures, duplicate structures, clipping, or runtime errors.

**PH-29** rebrands the application as **Protos Harvest 2.0**, installs responsive GPT Image 2 desktop/mobile title art and icon, rewrites English and Simplified Chinese title/control copy with exact key parity, and adds deterministic woodland day/night/rain music. Clearing weather has priority, unchanged selections do not restart, transitions retain the existing equal-power path, and legacy remote-biome two-track playback remains intact. All three PCM tracks import as long-form forward loops and ship in the verified Web pack.

### Phase 6 implementation evidence

**PH-30** adds twelve stable habitat definitions across desert, wetland, frozen, and volcanic regions. Population snapshots derive only from seed, day, time, and canonical ecology deltas; territorial creatures, nests, and herds use bounded capacities, leashes, schedules, depletion, and day-based respawn. Habitat entry, not elapsed ambush timers, materializes the same population after streaming or reload.

**PH-31** protects friendly wild herds from ordinary tool and Impact damage while retaining an explicit ecology-authority depletion path. Herd trust persists in bounded habitat records, daily renewable yields are exact-once, and the existing hostile fauna controller remains responsible for territorial large creatures and nest packs.

**PH-32 / PH-33** define seventeen named wilderness and hazard materials, five concrete farm-capability graphs, and per-tier routine value ceilings below equivalent farming income. Four deep-biome events each expose deterministic forecasts, visible telegraphs, preparation capabilities, bounded mitigation, stabilization windows, and exact-once resource tokens.

**PH-34** establishes the Sunken Crown lair, quest gate, safe exit, immutable Ironjaw first-clear ID, exact-once Burrow Core, home well presentation, and six deep-till cells. The first clear enables deep tilling and well capabilities; retries cannot grant another core or duplicate the clear record.

**PH-35** extends the cross-domain boundary with an atomic expedition-return candidate. Active-run scrap, cores, sorted cargo, optional remote-ruin activation, profile result, and run phase commit together or not at all. Existing home inventory is never subjected to excursion loss, remote activation is a canonical ecology token, and replay is rejected by the run summary and token ledgers.

### Phase 7 implementation evidence

**PH-36 / PH-37** complete the expansion gate on the exact Godot 4.7.2 tree. Direct import, repository-wide GDScript lint, all phase-focused contracts, the 1,984-check smoke suite, audio validation, bounded native boot, Web export, and exported-PCK boot pass without script, parse, resource, or runtime errors. The previous 3,360-day deterministic soak and maximum-ledger save-budget checks remain green, while the Phase 6 suite adds stream/reload habitat equivalence and exact-once expedition return.

The certified Web bundle contains non-empty HTML, JavaScript, WASM, and PCK artifacts. Large payloads are stored in Manus WebDev storage; the generated shell and audio worklets remain same-origin. The existing `proto-isometric-web` project passes TypeScript and production builds, reaches `title-ready` and `field-ready` over HTTP, fills the browser viewport with a borderless iframe, accepts representative input, and produces no browser-console errors. Final checkpoint `3c3baa9e` is the canonical WebDev handoff because no direct publish tool was available in this session.
