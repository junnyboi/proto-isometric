...
**P11 is complete.** P11 introduced fixes and certification infrastructure only. During its execution, other approved work landed on canonical `main`: the deterministic wild-flora gathering/farming loop, the inspectable-world dossier interface, localized outcome previews, and non-exported concept records. P11 integrated these changes without rewriting shared history and reran every affected release gate. The final certified runtime tree is `fab0f487d9061f74ac0d3794da9335d3dce53da7`; canonical `main` then fast-forwarded to docs-only revision `7eba623ba31b355b320748b690cd46ad43bd44b0`, which is excluded by the explicit Web export and therefore leaves runtime bytes unchanged. The annotated tag `protos-harvest-stewardship-p11` names the P11 evidence-closure commit.

Persistence certification now owns a true validator-valid simultaneous maximum rather than a nominal envelope. It contains **4,096 plots and 512 orchard trees**, measures **1,154,397 canonical bytes** against the **1,572,864-byte** hard limit, and hashes to `4ca2be374005abcd00f210c79a7d20443ab27a3cdec99a84c4cd898f159e015b`. Its retained newline-terminated fixture is 1,154,398 bytes with file SHA-256 `918bdea3f28c96e336f52f105d66d4d28a1abb2918f34f52814a35b41e68a4da`. The representative 100-day state is 15,820 canonical bytes and hashes to `f8d4ec2d5d70bf2470536cac30ee96d3ced804cf384410a91b0f44e3b4619aab`; its retained newline-terminated file is 15,821 bytes with SHA-256 `2da9ab753e7f3720a7f8626e4a98d0d05531950d640e2f3ae9cd9d6b10ab5859`. The exact prior P10 bundle and a 1,519-byte schema-4 neutral backup remain separately retained with detached checksums.

P11 closes every audit item found during certification. `SaveRepository` recovery now ranks validated candidates by hash-bound gameplay revision rather than mutable metadata sequence; regression coverage proves sequence-only tampering cannot outrank newer gameplay. The runtime font is reproducibly subset from licensed Noto Sans CJK SC Medium over every current English and Simplified Chinese locale codepoint. The generated runtime ledger binds **220 shipping binary assets** to exact size, SHA-256, decoder, dimensions, and provenance records. Standard verification fully decodes every PNG, WAV, Ogg, and OTF, checks all **1,093 required font codepoints**, rejects asset-inventory drift, and enforces the 1,000-line, 30-public-method, 20-return, and 100-column GDScript budgets. The legacy phase-three crop contract was updated from six to the approved eleven crops, and the P10 fishing capacity fixture now retains its required rod while filling all remaining slots; these fixes preserve the historical regression intent after the concurrent catalog expansion.

The final Godot **4.7.2 stable** release gate passes direct import, repository-wide lint, full binary integrity, BGM loops, source budgets across **340 GDScript files**, bounded boot, **2,142 smoke assertions**, the golden 1,000-day schedule, explicit Web export, exact exported-PCK boot, `[PCK_BOOT_PASS]`, and `[PASS] Protos Harvest --release`. The dossier additions pass **31 interaction**, **4 asset**, and **63 primitive** checks; Phase B passes **27 checks**. The complete historical matrix passes **21/21 runners**: interaction inspection **19**, interaction A/B/C **14/27/21**, Harvest P0/P1/P2/P3/P4/P6 **83/32/19/24/21/15**, P5 authorities/presentation **19/13**, and settlement P1/P2/P4/P5/P6/P7/P8/P9/P10 **46/24/39/27/32/22/22/22/27**.

Two fresh native and two fresh Chromium Web 1,000-day runs execute the shared schedule and converge on canonical hash `162fc7dec14149a3ea7beb67007b8be8cc474a310ecd87cacee19061b4f37270` at **15,933 canonical bytes**. The final integrated native runtime passes a 65-second full-heartbeat preflight followed by an uninterrupted **7,190-second** run, providing **7,255 seconds (120 minutes 55 seconds)** of coverage with zero failures. The long run plateaus at **4,896 objects** and **285,762,337 reported memory bytes**. Chromium and Firefox each pass **3,600 seconds** with **59 health heartbeats**, a stable 1280×720 canvas, representative input, and empty page-error, request-failure, and unexpected-console arrays.

The final HTTP browser matrix passes **eight cases** across Chromium and Firefox and five viewport classes: 1920×1080, 1280×720, 844×390, 390×844, and 768×1024. Every case reaches the field, receives HTTP success for JavaScript/WASM/PCK, exercises keyboard and trusted audio unlock, and reports no unexpected console or network failure. All six Chromium cases exercise a virtual standard controller, and both portrait cases exercise touch. Normal, private, and persistence-not-guaranteed storage modes, hard refresh, and tab suspension are covered. An isolated exported Godot Web harness uses the real browser filesystem to write a revision-2 primary and revision-1 backup, corrupt the primary, quarantine it, select the `.bak` candidate, and return recovered revision 1. The sandbox's expected `persistence_not_guaranteed` warning remains an honest capability notice and is not represented as durable-storage proof.

The final generated bundle contains **7,853-byte HTML**, **279,815-byte JavaScript**, **39,514,754-byte WASM**, and **76,121,880-byte PCK** artifacts. Their SHA-256 values are `f42f3ed451b7a9ef73759431677e080ece29a4a12850fedc71d87b66af483865`, `33c94cb3175f3333b82e2a3be5e8e86f77986f0aa2042b1631f6367a4e5bb6ba`, `fc74679e3b97f76878947fcd4fbe1268cbfa6188182a2e33bbc3f5dc9bfa57d0`, and `02ae3c34a81ae61a466beb0cef6fc2f38f8a6c175a2601e9997bf088b7caca21`, respectively. The same PCK is deployed from immutable Manus storage path `/manus-storage/proto-isometric_9a7ec588.pck`. The WebDev host keeps one zero-margin fullscreen iframe; only two generated JavaScript worklet resolvers are adapted to retain same-origin audio modules. Unit, TypeScript, and production builds pass after concurrent checkpoint integration.

Checkpoint `d08421be` is saved and automatically published to `https://walkers-wake-f5qxn5w7.manus.space/`. Production verification receives the exact final PCK mapping, loads the title, enters the field with trusted Enter input, opens the new dossier with trusted E input, renders the concurrent wild flora, and returns clean network/runtime behavior. Final report, exact bundle, fixtures, visual hashes, browser matrix, soak logs, prior-bundle retention, and recovery evidence are archived externally under the P11 evidence root. There are no unresolved high- or medium-severity P11 findings.
